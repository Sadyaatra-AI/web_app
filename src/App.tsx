...
import { Destination, DestinationType, FilterState, Region } from './types';
...
export function App() {
  ...
  const filteredDestinations = useMemo(() => {
    return apiDestinations.filter((dest) => {
      ...
      if (filters.maxBudget < dest.budgetTypical) {
        return false;
      }

      if (
        filters.mood !== 'All Moods' &&
        !dest.travelMoods.some((v) =>
          v.toLowerCase().includes(filters.mood.toLowerCase())
        )
      ) {
        return false;
      }
      ...
      if (
        ...
        !dest.destinationTypes.includes(filters.type as DestinationType)
      ) {
        return false;
      }
