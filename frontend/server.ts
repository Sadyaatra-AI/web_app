...
app.post('/api/chat', async (req, res) => {
  try {
    const { message, destinationContext, history } = req.body;
...
Structure your answers with clean paragraphs and bullet points for readability.`;
      ...
      const replyText = response.text || "I apologize, I couldn't generate a response at this moment.";
      ...
      if (query.includes('itinerary') || query.includes('day') || query.includes('plan')) {
        fallback = `Here is a curated itinerary tailored for **${destinationContext?.name || 'your journey'}**:\n\n` +
        ...
        fallback = `For **${destinationContext?.name || 'this trip'}**, a comfortable 3–4 day trip typically ranges between **${destinationContext?.budgetFormatted || '₹15,000 – ₹25,000 per person'}**.\n\n` +
        ...
        fallback = `Culinary highlights for **${destinationContext?.name || 'this region'}**:\n\n` +
        ...
        fallback = `Greetings! As your **Sadyaatra Curator**, I am here to guide your discovery of **${destinationContext?.name || 'exceptional travel destinations'}**.\n\n` +
